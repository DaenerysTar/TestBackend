...
public class AccountController {
     ...
     * @param longitude
     * @param zipcode
     * @param hospital_phone
     * @param outpatient_hour
     * @param introduction
    ...
    public ResponseEntity<Response<String>> addHospital2(@RequestParam String hospital_name, @RequestParam String level,
                                                         @RequestParam String address, @RequestParam BigDecimal latitude,
                                                         @RequestParam BigDecimal longitude, @RequestParam String zipcode,
                                                         @RequestParam String hospital_phone, @RequestParam String outpatient_hour,
     ...
     * 删除医院
     * @param hospital_id
     * @return org.springframework.http.ResponseEntity<edu.tongji.backend.util.Response < java.lang.String>>
     * user accessible api
...
//        医生对医院有外键依赖
    ...
    /**
     * 获取医生账号列表
     * @return edu.tongji.backend.util.Response<java.util.List < edu.tongji.backend.dto.DoctorInfoDTO>>
     */
     ...
     * <li>The first 6 digits must indicate a valid regional code,such as 310101<em>(31 Shanghai,01 Municipal region,
     * 01 Huangpu District(01 usually indicates the must central district))</em></li>
    ...
    public ResponseEntity<Response<Boolean>> editAccount(@RequestBody DoctorEditDTO doctoredit) {
        ...
        try {
            Doctor doctor = new Doctor(doctoredit.getDoctorId(), 0, idCard, doctoredit.getDepartment(),
                    doctoredit.getTitle(), doctoredit.getPhotoPath());
        ...
        return new ResponseEntity<>(Response.success(true, "The profile of doctor "
                + doctoredit.getDoctorId() + " has been updated!"), HttpStatus.OK);
    }
    /**
     * 管理员注册
     * @param inviteCode
    ...
    public ResponseEntity<Response<Boolean>> registerAdmin(@RequestParam String inviteCode, @RequestParam String name,
        ...
        if (hospitalId == null) {
            ...
            // The password must contain at least one digit, one lowercase, one uppercase and one special character,
            // the length should be between 8 and 16.
